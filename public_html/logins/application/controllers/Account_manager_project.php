...
class Account_manager_project extends CI_Controller {
	private $error = array();
	private $credit;
	function __construct() {
		...
		if (!$this->checkLogin()) {
			redirect(base_url());
		} else if ($this->session->userdata['user_type'] != 'ACCOUNT MANAGER') {
			redirect(base_url());
		}
	}
	public function index() {
		...
		$data['entry_project_manager_department'] = $this->lang->line('entry_project_manager_department');
		...
		$data['action'] = base_url() . 'account_manager_project';
		$data['project_candidate'] = base_url() . 'account_manager_project/minterviews';
		$data['edit_project'] = base_url() . 'account_manager_project/edit';
		$data['create_project'] = base_url() . 'account_manager_project/create';
		$data['remove_project'] = base_url() . 'account_manager_project/remove';
		$data['remove_all_project'] = base_url() . 'account_manager_project/removeall';
		$data['send_link_mail_url'] = base_url() . 'account_manager_project/sendLinkMail';
		$data['interview_status_url'] = base_url() . 'account_manager_project/checkInterviewStatus';

		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			$projects = $this->project_model->getAccountManagerProjectsBySearch($this->session->userdata['user'], $this->input->post('text_search'));
		} else {
			$projects = $this->project_model->getAccountManagerProjects($this->session->userdata['user']);
		}
		$data['projects'] = array();
		if ($projects) {
			foreach ($projects as $project) {
				$data['projects'][] = array(
					...
					'manager' => $this->manager_model->getManagerDetailByAuthID($project->manager_id),
		...
		$this->app->view('account_manager_project', $data);
	}
	...
	public function minterviews($project_id = '') {
		...
		$data['cancel'] = base_url() . 'account_manager_project';
		$data['action'] = base_url() . 'account_manager_project/minterviews/' . $project_id;
		...
		$this->app->view('account_manager_mview_interviews', $data);
	}
	...
	public function updateEvoRate() {
		if (count($_POST) == 2) {
			...
			if ($this->session->userdata['user_type'] == 'MANAGER') {
				...
				$data = array('account_manager_eva_rating' => $value);
			}
	...
	public function getComment() {
		if (count($_POST) > 0) {
			$interview_id = $this->input->post('interview_id');
			$type = $this->input->post('type');
			$field = "account_manager_eva_comment";
			$where = array('interview_id' => $interview_id);
			echo $this->interview_model->getComment($field, $where, $type);
		}
	}
	public function saveComment() {
		if (count($_POST) == 2) {
			...
			if ($comment == "" || $interview_id == "") {
				...
				$data = array("account_manager_eva_comment" => $comment);
				$where = array('interview_id' => $interview_id);
