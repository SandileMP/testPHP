...
class Manager extends CI_Controller {
	...
	public function index(){
		...
		$data['label_manager_department'] = $this->lang->line('label_manager_department');
		$data['label_manager_email'] = $this->lang->line('label_manager_email');
		$data['label_manager_status'] = $this->lang->line('label_manager_status');
	...
	public function create(){
		...
		if($this->input->server('REQUEST_METHOD') == 'POST' && $this->validateForm()){
			...
			$adddata['account_manager_id'] = $this->session->userdata['user'];
		...
		$data['entry_department'] = $this->lang->line('entry_department');
		...
		if(isset($this->error['error_department'])){
			$data['error_department'] = $this->error['error_department'];
		}else{
	...
	public function edit($manager_id = ''){
		...
		if($this->input->post('department')){
			$data['department'] = $this->input->post('department');
		}else if($data['manager']['department']){
			$data['department'] = $data['manager']['department'];
		}else{
			$data['department'] = '';
		}
		if($this->input->post('email')){
		...
		if($this->input->post('status')){
			$data['status'] = $this->input->post('status');
		}else if($data['manager']['status']){
			$data['status'] = $data['manager']['status'];
		}else{
			$data['status'] = 0;
		}
	...
	private function validateForm(){
		...
		if(empty($this->input->post('department'))){
			$this->error['error_department'] = $this->lang->line('error_department');
		}
