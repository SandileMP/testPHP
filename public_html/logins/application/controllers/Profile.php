...
class Profile extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->model(array('auth_model', 'account_manager_model', 'manager_model'));
        if(!$this->checkLogin()){
        	redirect(base_url());
        }
    }
	...
	private function validateForm(){
		...
		if($this->session->userdata['user_type'] == 'CLIENT'){
			$hr_id = $this->account_manager_model->getAuthAccountManager($this->session->userdata['user']);
			$already = $this->account_manager_model->checkDuplicateUpdateEmail($this->input->post('email'),$hr_id['profile_id']);
			if($already){
