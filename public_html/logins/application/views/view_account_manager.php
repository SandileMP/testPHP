		...
		<div class="container-fluid">
			<div class="panel panel-headline">
				<div class="panel-heading">
					<h3 class="panel-title"><?=$text_account_manager;?></h3>
				</div>
						...
						<table class="table">
							<thead>
								<tr>
									...
									<th><?=$label_account_manager_account_manager?></th>
									<th><?=$label_account_manager_name?></th>
									<th><?=$label_account_manager_email?></th>
									<th><?=$label_account_manager_phone?></th>
									<th><?=$label_account_manager_address?></th>
									<th><?=$label_account_manager_credit?></th>
									<th>Status</th>
									<th><?=$label_action?></th>
								</tr>
							</thead>
							<tbody>
								<?php if ($account_managers) {?>
								<?php foreach ($account_managers as $account_manager) {?>
								<tr>
									<td>
									<label class="fancy-checkbox">
										<input type="checkbox" name="removecheck[]" value="<?=$account_manager->account_manager_id?>" />
										<span></span>
									</label>
									</td>
									<td><?=$account_manager->account_manager?></td>
									<td><?=$account_manager->name?></td>
									<td><?=$account_manager->email?></td>
									<td><?=$account_manager->phone?></td>
									<td><?=$account_manager->address?></td>
									<td><?=$account_manager->credits?></td>
									<td> <btn class="label <?=$account_manager->status == 1 ? 'label-success' : 'label-danger'?>"><?=$account_manager->status == 1 ? 'Activate' : 'Deactivate'?></btn> </td>
									<td>
										<a href="<?=$edit?>/<?=$account_manager->account_manager_id?>" class="btn btn-info" data-toggle="tooltip" title="<?=$button_edit?>"><i class="fa fa-pencil"></i></a>
										<a href="<?=$remove?>/<?=$account_manager->account_manager_id?>" class="btn btn-danger remove" data-toggle="tooltip" title="<?=$button_remove?>"><i class="fa fa-trash"></i></a>
									</td>
								</tr>
								<?php }?>
								<?php } else {?>
