					...
					<table class="table table-striped table-bordered table-primary">
						<thead>
							<tr>
								...
								<th class="text-center"><?=$text_rater_evaluation?></th>
						...
						<tbody>
							<?php if ($interviews) { ?>
							<?php
                                foreach ($interviews as $mkey => $interview) {
                                $question = array();
							...
							<tr>
								...
								<td>
									...
									<label class=" red">
										...
										value="1"
										style="display: none;"
										<?=$interview['user_rater_evaluation']['rating'] == 1 ? 'checked' : ''?>
										onchange="updateRating(this.value,<?=$interview['interview_id']?>,<?=$interview['user_rater_evaluation']['rater_id']?>)"
										name="optradio[<?=$mkey?>]">
										<div class="layer"></div>
									...
									<label class=" orange">
										...
										value="2"
										style="display: none;"
										<?=$interview['user_rater_evaluation']['rating'] == 2 ? 'checked' : ''?>
										onchange="updateRating(this.value,<?=$interview['interview_id']?>,<?=$interview['user_rater_evaluation']['rater_id']?>)"
										name="optradio[<?=$mkey?>]">
										<div class="button"><span></span></div>
									</label>
									...
									<label class=" green">
										...
										value="3"
										style="display: none;"
										<?=$interview['user_rater_evaluation']['rating'] == 3 ? 'checked' : ''?>
										onchange="updateRating(this.value,<?=$interview['interview_id']?>,<?=$interview['user_rater_evaluation']['rater_id']?>)"
										name="optradio[<?=$mkey?>]">
										<div class="button"><span></span></div>
									</label><br>
									<a href="javascript:void(0)" onclick="addComment(<?=$interview['interview_id']?>,'RATER',<?=$interview['user_rater_evaluation']['rater_id']?>)">Add Comment</a>
									<?php }?>
								</td>
								<td>
									<?php if ($interview['interview_id']) {?>
									<?php if ($interview['manager_eva_rating'] == 1) {
                                        $class = 'red';
								...
								<?php if ($interview['other_rater_evaluation']) { ?>
								<?php foreach ($interview['other_rater_evaluation'] as $key => $rater) { ?>
								<td>
									<?php if ($interview['interview_id']) {?>
		    ...
		    <div class="modal-body">
				...
				<div class="form-group">
			    	...
			    	<div class="col-sm-10">
			      		...
			      		<input type="hidden" name="txt_rater_id" id="txt_rater_id">
			    	</div>
			  	</div>
		    </div>
		    <div class="modal-footer">
...
<script>
	...
	$(document).ready(function(){
		$("#saveCommentFrm").on('submit',function(e){
			e.preventDefault();
			$.ajax({
				url:"<?=base_url('rater_project/saveComment')?>",
				method:"post",
	...
	function updateRating(val,interview_id,rater_id)	{
		$.ajax({
			url:"<?=base_url('rater_project/updateEvoRate')?>",
			method:"POST",
			data:"rate="+val+"&interview_id="+interview_id+"&rater_id="+rater_id,
			success:function(data){
				if(data==0)
				{
	...
	function viewComment(x,type,id)	{
		$.ajax({
			url:"<?=base_url('rater_project/viewComment')?>",
			method:"POST",
			data:{
				"interview_id":x,
				"type":type,
	...
	function addComment(x,type,id) {
		$.ajax({
			url:"<?=base_url('rater_project/getComment')?>",
			method:"POST",
			data:{
				"interview_id":x,
				"type":type,
	...
	function sendEmail() {
		...
		if(templateId == undefined){
			...
			$("#etModalContent").html('<img src="<?=base_url()?>assets/img/preload.gif" style="margin-left:45%;">');
			$.ajax({
				url:"<?=base_url('rater_project/sendEmailToSelectedTemplate/')?>"+templateId,
				method:"POST",
