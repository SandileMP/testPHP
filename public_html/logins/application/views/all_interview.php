			...
			<div class="panel panel-headline">
                ...
                <form action="#" method="post" id="interviews-list-form">
					<table class="table table-striped table-bordered table-primary">
						<thead>
							<tr>
                            	<th><label class="fancy-checkbox"><input type="checkbox" name="interviewCheckAll" id="interviewCheckAll" /><span></span></label></th>
								...
								<?php for($rr = 1 ; $rr <= PROJECT_MAX_RATER ; $rr++): ?>
                                <th class="text-center">Rater <?= $rr ?></th>
								<?php endfor ?>
								<th class="text-center"><?=$manager_evaluation?></th>
							</tr>
						</thead>
						<tbody>
							<?php if ($interviews) { ?>
								...
								<tr>
									...
									<td><span class="label label_<?= $interview['interview_id'] ?> <?= interviewStatusColor($interview['status']) ?>"><?= interviewStatus($interview['status'])?></span><br>
									<span class="email_type<?=$interview['candidate_id']?> label label-info"><?=$interview['email_type']?></span></td>
									<td>
										<?php if ($interview['interview_id']) {?>
										<div class="watch_interview_container <?= ($interview['interview_id'] &&  $interview['status'] != 1 ) ? 'hide' : ''  ?>">
											<button type="button" data-interview=<?=$interview['interview_id']?> data-client="<?=$interview['account_manager_id']?>" data-credited="<?=$interview['is_credited']?>" class="btn btn-info watch_interview" data-toggle="tooltip" value="<?=$interview['path']?>" title="<?=$button_watch?>" data-videoduration="<?= $interview['total_time_taken'] ?>"><?=$button_watch?></button>
											<br><br>
									...
									<?php $count_rater = count($interview['rater_evaluation']); 
										if ($interview['rater_evaluation']) {
										foreach ($interview['rater_evaluation'] as $key => $rater) { ?>
										<td>
											<?php if ($interview['interview_id']) {
											$class = '';
											if ($rater['rating'] == 1) {
											...
											?>
											<label class=" <?=$class?>">
												<input type="radio" class="class_ev_rate" value="1" style="display: none;" disabled <?=$rater['rating'] ? 'checked' : ''?>>
												<div class="layer"></div><div class="button"><span></span></div>
											</label><br>
									...
									<?php } ?>
                                    <?php
                                    /// add empty td  to table
                                    $count_td = (PROJECT_MAX_RATER - $count_rater);
                                    $count_td = ($count_td > 0 ) ? $count_td : 0;
                                    echo str_repeat("<td></td>",$count_td);
                                    ?>
							    ...
							    <tr><td colspan="<?= (6 + PROJECT_MAX_RATER) ?>" class="text-center"><?=$text_empty?></td></tr>
							<?php }?>
						</tbody>
					</table>
                </form>
...
<style>
.button {display: inline-block;position: relative;width: 30px;height: 30px;margin: 10px;cursor: pointer;}
.button span {display: block;position: absolute;width: 30px;height: 30px;padding: 0;top: 50%;left: 50%;-webkit-transform: translate(-50%, -50%);-ms-transform: translate(-50%, -50%);-o-transform: translate(-50%, -50%);transform: translate(-50%, -50%);border-radius: 100%;background: #eeeeee;box-shadow: 0 2px 5px 0 rgba(0, 0, 0, 0.26);transition: ease .3s;}
.red input:checked ~ .button span {background: red;}
.orange input:checked ~ .button span {background: orange;}
.green input:checked ~ .button span {background: green;}
.button span:hover {padding: 10px;}
</style>
<!-- View Comment Modal -->
	...
	$(function(){
		$("#saveCommentFrm").on('submit',function(e){
			...
			$.ajax({
				...
				success:function(data){
					if(data==1) $("#saveResponse").html('<div class="alert alert-success fade in alert-dismissable"">\
						<a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>\
						<strong>Success!</strong> Your comment has been Added Successfully.\
					</div>');
					else $("#saveResponse").html('<div class="alert alert-danger fade in alert-dismissable"">\
						<a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>\
