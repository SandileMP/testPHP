...
class Candidate_model extends CI_Model {
	...
	public function removeCandidate($candidate_id = '') {
        $this->Action_model->deleteRecord($candidate_id,USER_CANDIDATE);
        /*
	...
	public function addRegisterCandidate($data = array()) {
		...
		if ($datas['candidate_id'] != 'null') {
		    ...
		    $old_candidate_id_list = $datas['candidate_id'] ? json_decode($datas['candidate_id']) : [];
            $new_candidate_id_list = $candidate_id ? [$candidate_id] : [];

			$candidates = array(
				'candidate_id' => json_encode(array_merge($old_candidate_id_list, $new_candidate_id_list)),
			);
		} else {
			$candidates = array(
	...
	public function addInvites($candidate_id = '', $project_code = '') {
		...
		$account_manager_name = $this->getClientByProject($data['project_id']);
		...
		$message .= "Website : <b>" . base_url() . "</b><br>";
		$message .= "Username : <b>" . $credentials->email . "</b><br>";
		$message .= "Password : <b>" . base64_decode($credentials->password) . "</b><br><br>";
		$message .= "Thank You<br><br>";
		$message .= "Account Manager<br>";
		$message .= $account_manager_name['account_manager'];
