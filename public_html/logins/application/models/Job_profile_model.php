...
class Job_profile_model extends CI_Model{
	public function getJob_profiles($manager_id = ''){
		$this->db->select('*');
	...
	public function getJob_profilesBySearch($manager_id = '',$serach = ''){
		$data = $this->db->query("SELECT * FROM tbl_job_profile WHERE manager_id = '{$manager_id}' AND title LIKE '%{$serach}%' OR role_title LIKE '%{$serach}%'");
		if($data){
	...
	public function getRoleProfileName($job_profile_id = ''){
		$this->db->select('role_title');
		$this->db->from('tbl_job_profile');
		$this->db->where('job_profile_id',$job_profile_id);
		$data = $this->db->get()->row_array();
		if($data){ return $data['role_title']; }
	}
}
?>