...
class Auth_model extends CI_Model{
	public function checkDetail($email,$password){
		...
		if($data){
			if ($data['type'] == 'CLIENT') {
				$client = $this->db->query("SELECT status FROM tbl_hr WHERE status = 1 AND email like '". $data['email']  ."'")->num_rows();
				if ($client == 1) {
					return $data;
				}
			}else if($data['type'] == 'CANDIDATE'){
				$candidate = $this->db->query("SELECT status FROM tbl_candidate WHERE status = 1 AND candidate_id = '". $data['profile_id']  ."'")->num_rows();
				if ($candidate == 1) {
					return $data;
				}
			}
