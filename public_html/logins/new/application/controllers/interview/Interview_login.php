...
class Interview_login extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->model(array('interview_model','candidate_model','project_model','auth_model'));
    	if($this->checkLogin()){
	...
	public function register(){
		...
		if($this->input->server('REQUEST_METHOD') == 'POST' && $this->validateRegisterForm()){
			if(!empty($_FILES['profile_image']['name'])){
				$create = $this->candidate_model->addRegisterCandidate($this->input->post(),$_FILES['profile_image']);			
			}else{
				$create = $this->candidate_model->addRegisterCandidate($this->input->post(),array());
			}
			if($create){
				redirect(base_url());
			}
		...
		if(isset($this->error['profile_image'])) $data['profile_image_error'] = $this->error['profile_image'];
		else $data['profile_image_error'] = '';

		if(isset($this->error['firstname'])) $data['firstname_error'] = $this->error['firstname'];
		else $data['firstname_error'] = '';

		if(isset($this->error['lastname'])) $data['lastname_error'] = $this->error['lastname'];
		else $data['lastname_error'] = '';

		if(isset($this->error['email'])) $data['email_error'] = $this->error['email'];
		else $data['email_error'] = '';

		if(isset($this->error['id'])) $data['id_error'] = $this->error['id'];
		else $data['id_error'] = '';

		if(isset($this->error['dob'])) $data['dob_error'] = $this->error['dob'];
		else $data['dob_error'] = '';

		if(isset($this->error['age'])) $data['age_error'] = $this->error['age'];
		else $data['age_error'] = '';

		if(isset($this->error['phone'])) $data['phone_error'] = $this->error['phone'];
		else $data['phone_error'] = '';
	...
	public function update(){
		...
		if($this->input->server('REQUEST_METHOD') == 'POST' && $this->validateRegisterForm()){
			$update = $this->candidate_model->updateRegisterCandidate($this->session->userdata['update_interview_user'],$this->input->post(),$_FILES['profile_image']);
			if($update){
		...
		if($this->input->post('firstname')) $data['firstname'] = $this->input->post('firstname');
		else if($candidate_detail['firstname']) $data['firstname'] = $candidate_detail['firstname'];
		else $data['firstname'] = '';

		if($this->input->post('lastname')) $data['lastname'] = $this->input->post('lastname');
		else if($candidate_detail['firstname']) $data['lastname'] = $candidate_detail['lastname'];
		else $data['lastname'] = '';

		if($this->input->post('email')) $data['email'] = $this->input->post('email');
		else if($candidate_detail['firstname']) $data['email'] = $candidate_detail['email'];
		else $data['email'] = '';
